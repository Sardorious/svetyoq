...
> 🔴 **58-sessiyadan KEYIN tasdiqlandi: prodda SQL jurnali HALI yoqiq.**
...
> parametrlari bilan yozmoqda, garchi 56-run buni tuzatgan hisoblansa ham.
> **Sabab uchta tekshiruv bilan aniqlandi:** (1) serverda `DB_ECHO=false`,
> `LOG_LEVEL=INFO` — sozlama emas; (2) konteynerda
> `grep -c engine_floor /app/app/core/logging.py` → **0**; (3)
> `git show HEAD:sveta/app/core/logging.py | grep -c engine_floor` → **0**,
> `git status -sb` → `main...origin/main`, `HEAD` = `c184648` (08-09 18:06).
> Ya'ni image o'sha commitdan yig'ilgan: `runner.py` fiksi unda **bor**
> (fon vazifalari ishlayapti — jurnalda `process_outbox` ko'rinadi),
> `logging.py` fiksi esa **yo'q** — u commitdan **keyin** yozilgan va hali
> commit qilinmagan.
> 👤 **Tartib muhim:** `.\push.ps1` → serverda `git pull` →
> `docker compose build sveta-api sveta-bot sveta-jobs` → `up -d`. Faqat
> qayta yig'ish yordam bermaydi (kod serverga yetib bormagan); uchala servis
> ham kerak, chunki `setup_logging(..., db_echo=...)` uchta kirish nuqtasida.
> Zarar: `reports` ga `INSERT` parametrlari bilan yoziladi, ya'ni
> `geom_exact` koordinatalari jurnalda qoladi va `purge_exact_geom` unga
> tegmaydi (`05` §3.2 buzilishi).
>
> ⚠️ **Yo'l-yo'lakay tuzatildi:** «55 run push qilinmagan» degan qayd
> **noto'g'ri edi** — repo origin bilan teng. Commit qilinmagani faqat
> 56-running 3- va 4-tuzatishi (`logging.py`, `NullPool`) va 58-run fayllari.
> `docker logs sveta-migrate` da ikkita «Context impl» bloki va bitta ham
> `Running upgrade` yo'q — bu xato emas, baza allaqachon head da.


> ⚠️ **57-sessiyadan KEYIN, prodda topilgan defekt (56-sessiya davomi).**
> `sveta-jobs` konteyneri cheksiz qayta ko'tarilardi va jurnalda faqat
> `{"logger": "__main__", "msg": "jobs.empty"}` bor edi. Sabab —
> `python -m app.jobs.runner` faylni `__main__` nomi bilan yuklaydi, vazifa
> modullari esa `from app.jobs.runner import JOBS` deb yozgan, ya'ni modul
> **ikkinchi marta** yuklanadi: `register()` lar kanonik nusxaga qo'shadi,
> `__main__` niki bo'sh qoladi. Natijada **oltita fon vazifasining birortasi
> ham ishlamagan** — xarita bo'sh, bildirishnoma yo'q, `territory_stats`
> to'lmagan, `geom_exact` tozalanmagan (maxfiylik). Nosozlik ikki tomondan
> jim edi: daraja `INFO` va chiqish kodi `0`.
> **Tuzatildi:** `runner.py` ning `__main__` bloki kanonik moduldan `main` ni
> import qiladi; `test_jobs_registry.py` ga ikkita qulf qo'shildi (mexanizm +
> kirish nuqtasi) va mutatsiya bilan tekshirildi — eski kodda yiqiladi.
> 👤 **Image qayta yig'ilishi shart:** `docker compose build sveta-jobs`
> → `docker compose up -d`; jurnalda `jobs.empty` emas, `jobs.start` va
> oltita vazifa nomi ko'rinishi kerak.
> **Uchinchi defekt — o'sha konteyner tuzalgandan keyin ko'rindi.** Jurnal
> butunlay SQLAlchemy bilan to'lgan edi (`BEGIN` / `SELECT … FOR UPDATE SKIP
> LOCKED` / `COMMIT`, har besh soniyada), `DB_ECHO` esa `false`. Sabab:
> `echo=False` SQLAlchemy loggeriga daraja **qo'ymaydi** — u ildizdan meros
> oladi va `LOG_LEVEL=INFO` ning o'zi yetarli. `setup_logging` dagi
> «jim qilish» qatori (`max(logging.INFO, root.level)`) hech qachon jim
> qilmagan: `INFO` da ham, `DEBUG` da ham natija `20`. Zarari ikki xil —
> **maxfiylik** (operator parametrlari bilan yoziladi, ya'ni `reports` ga
> `INSERT` `geom_exact` koordinatalarini jurnalga tushiradi va
> `purge_exact_geom` unga tegmaydi) va **hajm** (`process_outbox` bo'sh
> navbatda ham kuniga ~50 000 satr). Tuzatildi: `sqlalchemy.engine` endi
> `DB_ECHO` bilan boshqariladi, uchala kirish nuqtasi sozlamani uzatadi;
> yangi `tests/test_logging_setup.py` — 8 ta qulf, mutatsiya bilan
> tekshirilgan. Testlarda ko'rinmasligining sababi: `setup_logging` ni
> birorta test chaqirmasdi.
> Shu bilan birga `sveta-db` healthcheck i tuzatildi
> (`pg_isready -h 127.0.0.1`, `start_period: 30s`) — usiz birinchi
> `docker compose up` da `sveta-migrate` `Connection refused` bilan yiqilardi.
> **To'rtinchi — CI birinchi marta yurdi va `requires_db` ning hammasi
> yiqildi** (26 failed, 98 errors), lekin hammasi **bitta** sababdan:
> `RuntimeError: … got Future … attached to a different loop`.
> `app/db/session.py` ning `_engine` i protsess davomida bitta, `pytest-asyncio`
> esa (`asyncio_mode = "auto"`) har testga **yangi event loop** beradi —
> poolda saqlangan `asyncpg` ulanishi birinchi testning loopiga bog'langan
> qoladi. **Yechim:** `APP_ENV=test` da `poolclass=NullPool` (SQLAlchemy
> hujjatining aynan shu holat uchun tavsiyasi; `pool_size` ni u bilan birga
> berib bo'lmaydi — `create_engine()` yiqiladi). Prodda pool o'z holicha.
> **Nima uchun 212 ta bazali test yozilib, bu faqat bugun ko'rindi:**
> sandboxda PostGIS yo'q va `conftest._db_reachable()` ularni o'tkazib
> yuboradi — ular birinchi marta faqat CI da uchrashdi.
> Uchta qulf `tests/test_config.py` da. ⚠️ Bu tuzatishni sandboxda tekshirib
> bo'lmaydi (baza yo'q) — isbot CI ning keyingi yurishida.
> Yakuniy: `pytest -m "not requires_db"` → **1356 passed, 1 skipped**,
> `ruff check` → toza.

**2026-08-09 (58-sessiya)** — ✅ **`06` §12 oltin ssenariylari endi nomlar
darajasida emas, **mazmuni** bilan bajariladi. Defekt topilmadi; sakkizta
mutatsiya bilan tekshirildi.**
✅ **Sandbox ishladi** — 56-ning `/tmp/sv56` muhiti va `/tmp/wg-libs/bin/ruff`
o'rnida: `pytest -m "not requires_db"` → **1375 passed, 1 skipped, 212
deselected**; `ruff check app tools tests alembic` → **toza**.

- **Nima uchun §12.** 57-sessiya uchta ochiq joy qoldirdi (§11, §12,
  `05` §3.1/§4.4–4.5). §12 tanlandi, chunki bo'shlig'i eng aniq
  o'lchanadigan edi: 46-sessiya raqamlarni test **nomlariga** bog'lagan,
  ssenariylarning **sonlari** esa o'sha testlarga qo'lda ko'chirilgan —
  `18`, `5`, `7`, `45` literal sifatida test kodida yotardi.
- **Uchta jim yo'nalish.** (1) Hujjatdagi son o'zgarsa ikkala mavjud test
  ham yashil qolardi (46 kalit so'zni tekshiradi, xulq-atvor testi esa o'z
  literalini). (2) §12.7 ning «`scale_capped = true`» i **vakuum** bo'lishi
  mumkin edi: `raw_scale` o'zi `local` bo'lsa bayroq hech narsa haqida
  bo'lardi va test o'zgarmasdan o'tardi. (3) §12.11 ning «**hech qachon**
  `local` dan oshmaydi» miqdor belgisi bitta nuqta bilan o'lchanardi.
- **Yechim — hujjat kirish ma'lumotining manbai.** Har qatordan son, kod
  nomi va kutilgan natija parse qilinadi va **o'sha qiymatlar bilan**
  `evaluate`, `decide`, `evaluate_status`, `confidence` yurgiziladi. §12.8
  uchun «chegara 7» beradigan `A_local` `06` §4.2 formulasi orqali
  **qidiriladi**; §12.9 uchun eng og'ir ikki manba `06` §2 jadvalidan
  **og'irligi bo'yicha** tanlanadi; §12.7 uchun «kam qamrov» =
  `guard.min_active_district - 1` (`06` §5.4 ning ta'rifi). §12.11 endi
  240 kombinatsiya bo'ylab yuriladi.
- **Vakuumga qarshi uchta tasdiq:** to'siq bo'lmaganda masshtab `local`
  **emas**; chegaraga yetgan o'sha xabarlar tasdiqlaydi; siyrak hududda
  o'sha 5 ta xabar yetarli (ya'ni «zich» so'zi ishlayapti).
- **Mutatsiya bilan tekshirildi** — defekt topilmagani uchun testlarning
  o'zi buzildi: hujjatda `5`→`8`, `45`→`90`, yangi §12.14 qatori; kodda
  `fingerprint` dan `r.scale`, `coverage_cap` ning ikkita sharti, `assign`
  dagi `layer=`, `LOW_CONFIDENCE_AFTER_MIN`. Har biri aynan bitta testni
  yiqitdi.
- **57-running bitta qaydi noto'g'ri edi.** «`45` esa **faqat** §8 da
  yashaydi» — aslida u §12.12 da ham bor. Ikkala qator alohida tahrir
  qilinadi, biri o'zgarib ikkinchisi qolsa hujjat **o'zi ichida** ziddiyatli
  bo'lardi; endi ikkalasi solishtiriladi.
- **👤 Hujjat savoli.** §12.12 «45 daqiqadan keyin `resolved`» deydi, §8 esa
  yopilish uchun **ikkita** shartni talab qiladi (`confidence < 40` **va**
  45 daqiqa sukut). Kod §8 ga ergashadi (to'g'ri), lekin §12 ni yolg'iz
  o'qigan odam uchun bu tuzoq. Hujjat qonun — agent uni tahrir qilmadi,
  savol «Ochiq savollar» da.
- **Rad etilgan:** 46-sessiyaning faylini kengaytirish (u ro'yxat/nom
  qatlami; mazmun qo'shilsa fayl ikkita savolga javob berardi —
  41-sessiyaning sabog'i); `05` §9.3 dagi 1–6 ssenariylarni ham shu yerda
  yurgizish (ular klasterlash quvuriga tegadi, bazasiz bajarib bo'lmaydi);
  §12.12 qatorini hujjatda to'ldirish (spetsifikatsiya — qonun).

Batafsili [58-sessiya faylida](58_oltin_ssenariylar_mazmuni_c160560e.md).

<details>
<summary>57-sessiya (`06` §8 deeskalatsiya kontrakti)</summary>


**2026-08-09 (57-sessiya)** — ✅ **`06` §8 deeskalatsiya bo'limi endi
hujjatdan o'qiladi, va **haqiqiy defekt** topildi: `apply_deescalation`
qoidani **inkor** bilan yozgan edi.**
✅ **Sandbox ishladi** — 56-ning `/tmp/sv56` muhiti butun holda qoldi:
`pytest -m "not requires_db"` → **1343 passed, 1 skipped, 212 deselected**;
`ruff check app tools tests alembic` → **toza** (54-rundan beri birinchi marta
`ruff` ham, `pytest` ham bitta runda yashil).

- **Nima uchun §8.** `EpicProgress.md` §3 uchta ochiq joy qoldirgan edi
  (§8, §11, §12). §8 tanlandi, chunki u `06` da **yagona** bo'lim bo'lib,
  formula emas, **vaqt o'tishi bilan nima o'zgarishini** aytadi — uning
  artefaktlari son emas, qoidalar, va shuning uchun jimgina buziladi.
- **Defekt: qoida inkor bilan yozilgan.** Hujjat «masshtab pasayishi …
  faqat `pending` da» deydi, kod esa `status == "confirmed"` ni tekshirardi,
  ya'ni `resolved`/`rejected`/`merged` ham pasayishga **ruxsat olardi**.
  Ochiq statuslar ikkitagina bo'lgani uchun natija bir xil ko'rinardi va
  hech qanday test yiqilmasdi — `evaluate` yopiq hodisada `is_open`
  qo'riqchisida qaytadi. Endi shart tasdiq orqali:
  `status != PENDING_STATUS`; tanimagan status ham pasaytirmaydi.
  Xulq-atvor haqiqiy chaqiruv joyida **o'zgarmadi**.
- **`45` daqiqa birinchi marta bog'landi.** 53-sessiya `40` ni bog'lagan
  (u §6 bandining chegarasi), `45` esa **faqat** §8 da yashaydi va
  `status.py` ga qo'lda ko'chirilgan edi.
- **Ikkita yangi invariant.** (1) `45 < autoclose (120)` — teng yoki katta
  bo'lsa so'nish qoidasi **o'lik kodga** aylanardi, chunki `evaluate_status`
  autoclose ni oldinroq ko'radi, va buni bironta xulq-atvor testi
  ko'rsatmasdi. (2) so'nish sababi ≠ autoclose sababi — ikkalasi ham
  `resolved` beradi, sabab qo'shilsa jurnal bo'yicha **nega** yopilgani
  aniqlanmasdi.
- **Ikki hujjat birinchi marta solishtirildi.** §8 sarlavhasidagi
  `(evaluate_outages, 60 s)` `05` §8 jadvalida ham bor; 45-sessiya
  `05` tomonini qulflagan, lekin ikkalasi bir-biri bilan hech qachon
  taqqoslanmagan edi.
- **«Yangi xabar → `W`, `scale`, `confidence`»** endi AST bilan o'lchanadi:
  uchala nom `evaluate` ning `values` lug'atida bo'lishi shart. Bittasi
  tushib qolsa hech qanday xato chiqmasdi — hodisa eski son bilan
  yashayverardi.
- **Rad etilgan:** 2-qatorni («freshness ↓ → confidence ↓») bu yerda ham
  tekshirish (`test_confidence_contract.py` da bor — takrorlash tuzatish
  joyini noaniq qilardi); `40` ni qayta bog'lash; `apply_deescalation` ni
  `OutageStatus` qabul qiladigan qilish (interfeys o'zgarishi, `06` talab
  qilmaydi); `ruff format` (82 fayl qayta formatlanardi, CI esa faqat
  `ruff check` yuradi — «Ochiq savollar» ga yozildi).

Batafsili [57-sessiya faylida](57_deeskalatsiya_kontrakti_3ad002c7.md).

</details>

<details>
<summary>56-sessiya (`06` §10 sxema o'zgarishlari)</summary>


**2026-08-09 (56-sessiya)** — ✅ **`06` §10 sxema o'zgarishlari endi
hujjatdan o'qiladi, uchala tomonda: hujjat ↔ model ↔ `0003`.**
⚠️ **INFRA-1 — yo'l topildi:** paketlar `~/.local` ga sig'maydi (kvota), lekin
`pip install --target /tmp/<nom>` ishlaydi; Python 3.10 uchun `sitecustomize.py`
da `enum.StrEnum`/`datetime.UTC` shimi kerak. Natija: **1325 passed, 1 skipped,
212 deselected** (1296 + yangi 29). `ruff` uchun joy qolmadi.
⛔ **Serverda `docker compose up` yiqildi** — `sveta-migrate`
`ConnectionRefusedError` `db:5432` ga; sabab **healthcheck poygasi**, tuzatildi.

- **§10 — `06` ning yagona bo'limi bo'lib, u formula emas, DDL beradi.**
  Sakkizta `ALTER TABLE ... ADD COLUMN` uch joyda takrorlanadi (hujjat,
  model, `0003`) va bugungacha hech biri boshqasidan o'qilmasdi.
- **`tests/test_schema.py` ning `ADDED_BY_06` lug'ati faqat nomlarni
  qulflardi.** Tip, `NOT NULL`, `DEFAULT`, `REFERENCES` — umuman
  o'lchanmagan: `numeric(6,1)` `numeric(4,1)` ga aylansa test yashil
  qolardi. Nusxa o'chirilmadi, lekin endi manba bilan solishtiriladi.
- **Model tipi ↔ migratsiya tipi eng jim tomon.** Test bazasi
  `alembic upgrade head` bilan quriladi, ya'ni **migratsiyaning** tipi
  haqiqiy ustunga aylanadi, ORM esa **modelnikini** ishlatadi; ikkalasi
  ajralsa farq faqat haqiqiy bazada, overflow paytida bilinardi.
- **Ikkita nasriy da'vo bog'landi** (55-ning «son nasrda yashaydi»
  naqshining davomi): «**`weight` va `required_score` qotiriladi**» endi
  DDL ning `NOT NULL` **siz** ustunlar to'plamiga aynan teng bo'lishi
  shart, va `scale_capped = true` jumlasi ustunning `boolean` +
  `DEFAULT false` ekanini asoslaydi.
- **Qotirish joylari ham qulflandi:** `create_report` da
  `weight = freeze_weight(...)` → `Report(..., weight=weight)`,
  `evaluate` da `"required_score": result.required_score`.
  Yo'l-yo'lakay `WEIGHT_DECIMALS` `numeric(3,1)` ning kasr qismiga
  bog'landi.
- **Defekt topilmadi — uchala tomon rozi.** Run holatni qulfladi,
  `app/` ga tegilmadi.
- **`pytest` o'rniga nima qilindi.** Faylning stdlib ga tayanadigan
  qismi (hujjat parseri, migratsiya AST i, `ADDED_BY_06`, nasr
  regexlari, manba matnidagi qulflar) alohida skript bilan ishga
  tushirildi va hammasi o'tdi; bitta xato shu yo'l bilan topildi —
  `` `scale_capped = true` `` da backticklar butun ifodani o'raydi,
  naqsh tuzatildi. ORM tomoni faqat matn bo'yicha solishtirildi.
- **Rad etilgan:** `outage.scale.capped` i18n kalitining ulanmaganini
  bu yerda ham tekshirish (41-sessiyaning `KNOWN_UNREACHABLE` ida
  turibdi — ikkinchi joyda takrorlash tuzatish joyini noaniq qilardi);
  `from tests.test_schema import ADDED_BY_06` (repoda testlararo import
  yo'q — `ast` bilan matn o'qildi); boshqa migratsiyalar bu ustunlarga
  tegmasligini tekshirish (`EXPECTED_COLUMNS` allaqachon ushlaydi).

- **Deploy defekti (👤 serverdan keldi).** Yangi volume bilan birinchi
  `docker compose up`: DB `Healthy`, `sveta-migrate` esa
  `ConnectionRefusedError: Connect call failed ('172.18.0.8', 5432)`.
  Sabab — postgres entrypoint i `initdb` va PostGIS init skriptlarini server
  **faqat unix soketda** turgan holda bajaradi (`listen_addresses=''`), hostsiz
  `pg_isready` esa o'sha soketga ulanib «accepting connections» deydi: compose
  konteynerni erta `healthy` deb belgilaydi. Vaqtlar buni tasdiqlaydi — DB
  10.1 s da «healthy», `migrate` 12.5 s da yiqilgan. **Tuzatildi:**
  `pg_isready -h 127.0.0.1 …` + `start_period: 30s`. 👤 Serverdagi
  `~/deploy/docker-compose.yml` — **repodagidan boshqa fayl**, unga qo'lda
  ko'chirish kerak; hozircha `docker compose up -d` ni qayta yurgizish yetarli.

Batafsili [56-sessiya faylida](56_sxema_kontrakti_370bc693.md).

</details>


<details>
<summary>55-sessiya (`06` §7 ishlangan misollar)</summary>
