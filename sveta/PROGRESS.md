...
| **Oxirgi run** | 2026-08-09 (57-run: `06` §8 deeskalatsiya kontrakti — `45` daqiqa, `(evaluate_outages, 60 s)` ning `05` §8 bilan pariteti, `W`/`scale`/`confidence` ning qayta hisoblanishi, «faqat `pending`» qoidasi va nasrdagi moderator yo'li; `apply_deescalation` defekti tuzatildi; **1343 passed + `ruff` toza**) |
| **Bloklangan** | ⚙️ **INFRA-1 — hozircha chetlab o'tilmoqda.** Ildiz disk yana 100% (22 MB bo'sh), `pip install` imkonsiz; 56-ning `/tmp/sv56` muhiti va oldingi runlardan qolgan `/tmp/wg-libs/bin/ruff` qayta ishlatildi va ikkala tekshiruv ham yurdi. `/tmp` dagi ~3 GB **boshqa uid** ga tegishli, agent o'chira olmaydi — 👤 `cleanup-sessions.ps1` ni har run oldidan yurgizing. ⛔ **CI hali hech qachon yurmagan** — 56 run push qilinmagan, 212 ta `requires_db` testi faqat CI da yuriladi. |
